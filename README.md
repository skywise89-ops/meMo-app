...
- 검색 완료 시 전체 메시지의 정렬된 타임라인을 유지하고, 결과 선택 시 그 타임라인의 실제 위치로 이동
- 목표 메시지 전후 100개를 먼저 표시한 뒤 위·아래 스크롤에 따라 100개씩 끊김 없이 연속 로드
- 전후 문맥 창과 최신 채팅을 섞지 않아 중간 날짜가 건너뛰는 현상 방지
- 앨범의 `대화 보기`도 전체 타임라인을 불러온 뒤 같은 방식으로 원본 시점에 고정
- 전체 메시지를 한 번에 DOM에 넣지 않아 2만 건 이상 대화에서도 초기 렌더링 부담 제한
